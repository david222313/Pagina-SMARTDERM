...
// La API key vive en el backend proxy (server.js) – NUNCA en el frontend
// getApiKey() se mantiene como stub vacío para compatibilidad
...
- Programa Día 1: Bienvenida, Barrera cutánea y microbioma, Fotoprotección 2025, Muestra comercial, Retinoides en la práctica, Ácidos en cosmética, Hiperpigmentación
...
async function analyzeProduct(productText, additionalContext) {
    const cfg = {}; // config desde Supabase en v3
...
// ── LLAMADA A LA API VÍA PROXY (no expone la key) ────────────────
// La key de Claude vive en el servidor, nunca en el frontend.
// El proxy está en: API_PROXY_URL (definido en script.js)
async function callClaude(userMessage, conversationHistory, courseContext) {
    // Modo demo si el proxy no está configurado
    const proxyUrl = (typeof API_PROXY_URL !== 'undefined' && API_PROXY_URL && !API_PROXY_URL.includes('TU_'))
        ? API_PROXY_URL
        : null;
    ...
    if (!proxyUrl) {
        // MODO DEMO: respuesta simulada sin API
        return {
            ok: true, demo: true,
            text: `👋 **DermBot en modo demo** (proxy no configurado).

Recibí tu consulta: *"${userMessage.slice(0,80)}${userMessage.length>80?'…':''}"*

Para respuestas reales, configura el backend proxy en \`API_PROXY_URL\` dentro de \`script.js\`.

🔬 Recuerda siempre consultar a un dermatólogo certificado para diagnósticos médicos.`
        };
    }
    ...
    try {
        const response = await fetch(`${proxyUrl}/api/chat`, {
            ...
            body: JSON.stringify({
                messages,
                systemPrompt: buildSystemPrompt(window.location.pathname, courseContext)
            })
        });

        if (!response.ok) {
            const err = await response.json();
            return { ok: false, text: `Error del asistente: ${err.error || response.statusText}` };
        }
